import hashpumpy
from server_vulnerable import generate_mac as server_generate_mac, verify as server_verify
import hashlib

def brute_force_key(intercepted_message: bytes, intercepted_mac: str, wordlist: list[str]) -> str | None:
    for key in wordlist:
        print(f'Trying key "{key}" ...', end=' ')
        trial_mac = hashlib.md5(key.encode() + intercepted_message).hexdigest()
